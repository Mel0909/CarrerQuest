...
def gerar_com_reportlab(caminho, destinatario, plano_estudos):
    ...
    # 🌌 Fundo mágico: preto/roxo escuro
    c.setFillColor(Color(0.05, 0.02, 0.15))  # Roxo escuro
    c.rect(0, 0, largura, altura, fill=1, stroke=0)

    # 🔮 Título principal - Dourado brilhante
    c.setFont("Helvetica-Bold", 20)
    c.setFillColor(Color(1, 0.85, 0.2))  # Amarelo dourado
    c.drawCentredString(largura / 2, altura - 70, "🔮 PLANO DE ESTUDOS MÁGICO 🔮")

    # 👤 Destinatário
    c.setFont("Helvetica-Bold", 14)
    c.setFillColor(Color(0.9, 0.9, 1))  # Branco azulado
    c.drawCentredString(largura / 2, altura - 100, f"Para: {destinatario}")

    # 🕰️ Data
    c.setFont("Helvetica", 10)
    c.setFillColor(Color(0.7, 0.7, 0.9))  # Cinza azulado
    ...
    # ✍️ Conteúdo do plano
    c.setFont("Helvetica", 10)
    c.setFillColor(Color(0.9, 0.95, 1))  # Branco suave
    ...
    for paragrafo in plano_estudos.strip().split("\n"):
        ...
        if y_atual < 100:
            c.showPage()
            # Repete fundo escuro na nova página
...
class PDFMagico(FPDF):
    def header(self):
        ...
        self.set_text_color(255, 220, 100)  # Amarelo dourado
